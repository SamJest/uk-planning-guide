...
| Eight opaque sitemap shards | partly repaired | First candidate removes only ten proved technical exclusions. Family-level partitioning remains a later sitemap-only cohort; no quality-unreviewed URL is removed. |

Validation: 32 tests pass with no skips; 24-route clean canary passes; three semantic repairs plus the four-capability journey pass; 27/27 registered official source URLs are reachable. Production deployment: none.
